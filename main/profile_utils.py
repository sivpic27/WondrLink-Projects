...
def safe_extract_value(data: Any, path: str, default: str = "unspecified") -> str:
    ...
    Returns string representation or default if not found/not convertible.
    """
    if not data:
        return default
    ...
    try:
        ...
        if isinstance(current, (str, int, float)):
            ...
            # For dict, try to extract meaningful summary
...
def extract_current_symptoms(profile: Dict) -> List[str]:
    ...
    # Look for current toxicities in active treatments
    ...
    # Also check for any direct symptoms field
...
def extract_patient_context_complex(profile: dict) -> Dict[str, Any]:
    ...
    like the colorectal cancer example provided.
    """
    if not profile:
        return {}
    ...
    if isinstance(patient_info, dict):
        ...
        if age is not None:
            context['age'] = age  # Store as integer
        else:
            context['age'] = None

        # Also try to get patient name
    ...
    # Current symptoms (from toxicities)
    ...
    if isinstance(current_options, list) and current_options:
        context['treatment_options'] = current_options[:3]  # First 3 options
...
def format_patient_summary_complex(context: Dict[str, Any]) -> str:
    ...
    if treatments:
        ...
        if current_tx:
            summary_parts.append(current_tx[0])  # Show the active treatment
